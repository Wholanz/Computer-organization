...
int main(int argc, char **argv)
{
    ...
    memcpy(new_path, "F:/xilinx/10.1/ISE/lib/nt;", 26);
    memcpy(new_path + 26, path, path_size + 1);
    SetEnvironmentVariable("PATH", new_path);
    free(new_path);

    sim_lib = LoadLibrary(TEXT("isim/_tmp/work/pbdebounce_isim_beh.exe_lib.nt.dll"));
    if (!sim_lib)
    {
        fprintf(stderr, "Could not open simulation library isim/_tmp/work/pbdebounce_isim_beh.exe_lib.nt.dll\n");
        return 1;
    }
